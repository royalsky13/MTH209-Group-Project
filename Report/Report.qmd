...
Fitting Logistic Regression for predicting the probability of the occurance of different types of cancer for an individual.

Cancer types according to different sites also bear very much importance. When it comes to the interest of the study, one of the most common question may arise that what is genetic effect on those cancer types. So, here we have 5 types of Cancer, namely, BRCA, COAD, KIRC, LUAD, PRAD. Now by Principle Component Analysis we have seen that the first 500 genes captures the maximum variation. Now we can check whether these genes have any significant effects on these cancer types.

So, Here we consider the Cancer Types as different labels and corresponding gene expression values with corresponding genes. Now we are interested in fitting a Multinomial Logistic Regression Model. let us consider the following table as defining the probabilities of occurrence with respect to the corresponding cancer types.

```{=tex}
\begin{table}[ht]
\centering
\caption{Cancer Types with Corresponding Probabilities for jth individual}
...
\end{table}
```
Now, by this we can observe that $\sum_{i=1}^{5}\pi_{ij} = 1$ so, $\pi_{5j} = 1-\sum_{i=1}^{4}\pi_{ij}$, considering 5th category as Pivotal Category. Therefore it is enough to estimate $\pi_i$ ; $i=1,2,3,4$ and we can automatically get the estimated value of $\pi_5$ and that will also reduce our manual labor.

\underline{\textbf{Model :}} Let $Y_j$ be the random variable of the occurrence of a type of cancer of jth person, \begin{equation*}
    Y_j =
    \begin{cases}
...
Therefore, $P(Y_j = i) = \pi_{ij} = \frac{e^{\boldsymbol{X^T_j\beta_i}}}{1+e^{\boldsymbol{X^T_j\beta_i}}}$ ; $i = 1,2,3,4$
...
Here by the p-values, we can see that gene 1 has significant effects in causing BRCA and KIRC, at level 0.05.

Now, let us fit the model and obtain the predicted probabilities of different types of cancer.

Let us consider two individuals with gene expression values of gene 1, 3.4678533 and 2.9411814. Then the fitted probabilites are as follows
...
\end{table}
```
From this we can see that for the first individual, according to the gene expression value of gene 1, there is greater probability of occurring BRCA. For the second person, the scenario is more or less same.

Again let us check it for gene 1 and 2 combinely,

Now, let us fit the model and obtain the predicted probabilities of different types of cancer.

Let us consider two individuals with gene expression values of gene 1 and gene 2 are as follows, 3.4678533, 2.9411814 and 2.9411814, 2.6632763. Then the fitted probabilites are as follows,
