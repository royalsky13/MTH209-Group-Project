...
format: 
  pdf:
    thanks: We express our sincere gratitude to Prof. Subhajit Dutta for his supervision and continuous support to complete this project.
...
-   $\bar{X_n} \stackrel{asym}{\sim} \mathcal{N}(\mu,\,\tfrac{\sigma^{2}}{n})$ i.e.,$\dfrac{\sqrt{n}(\bar{X_n}-\mu)}{\sigma}\xrightarrow[]{d} \mathcal{N}(0,\,1) \text{ as } \boxed{n \rightarrow \infty}$ (by [Central Limit Theorem]{.underline}) where $\bar{X_n}$ is a random variable with mean $\mu$ and variance $\tfrac{\sigma^2}{n}$ that denotes mean gene expression for a particular gene.
